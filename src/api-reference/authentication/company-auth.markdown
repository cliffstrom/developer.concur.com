...
Auth tokens are valid only for 12 hours, so partners have 12 hours to exchange the auth token for a refresh and access token. Partners can use this auth token multiple times within the 12 hours in case of network failure.
